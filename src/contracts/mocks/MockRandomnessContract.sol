...
contract MockRandomnessContract is VRFConsumerBase {
    ...
    function randomNumber(uint256 userProvidedSeed) public returns (bytes32 requestId) {
        ...
        // Dont call requestRandomness(keyHash, fee, userProvidedSeed).
        // Instead, mock the random number;
        // return requestRandomness(keyHash, fee, userProvidedSeed);

        requestId = bytes32("my random request Id");
        reqId = bytes32("my random request Id");

        fulfillRandomNumber(requestId, userProvidedSeed);
        return requestId;
    }

    function fulfillRandomNumber(bytes32 _requestId, uint256 _randomness) public {
        reqId = _requestId;
